...
import { animateDeck } from "@/experiments/video-studio/server/animateDeck";
...
export async function POST(request: Request) {
  ...
  const rateLimit = checkRateLimit(`video-studio-animate:${ip}`, {
    max: 6,
    windowMs: 120_000,
  });
  ...
  // Fire-and-forget. The job store is the channel the client polls on.
  void (async () => {
    updateJob(jobId, {
      status: "rendering",
      progress: 0.02,
    ...
    try {
      const result = await animateDeck({
      ...
      updateJob(jobId, {
        ...
        progress: 1,
        message:
          error instanceof Error ? error.message : "Animation failed unexpectedly.",
        error:
          error instanceof Error ? error.message : "Animation failed unexpectedly.",
        completedAt: Date.now(),
      });

...
export function VideoStudio() {
  ...
  // Source toggle (upload vs URL) + URL flow inputs. The site URL stays in
  // state across mode toggles so the user doesn't lose what they typed if
  // they switch away accidentally.
  const [sourceMode, setSourceMode] = useState<SourceMode>("upload");
  const [siteUrl, setSiteUrl] = useState("");
  // Free-form design.md baseline. Generated by the URL analyzer, or
  // pasted/uploaded by the user. Carried into the animate prompt as a
  // DESIGN SYSTEM block that both engines honor.
  ...
  // URL flow: ask the server to fetch + analyze the site, then drop the
  // returned scene plan + design.md into studio state. Reuses the same
  // animation reset path as the .pptx upload so the user can re-animate
  // immediately.
  const handleAnalyzeUrl = useCallback(async () => {
    const trimmed = siteUrl.trim();
    if (trimmed.length === 0) return;

    setRenderStatus(`Reading ${trimmed}…`);

    try {
      const response = await fetch("/api/experiments/video-studio/url", {
        method: "POST",
      ...
      if (!response.ok || !payload.scenePlan) {
        throw new Error(payload.error ?? "Couldn't analyze that URL.");
      }

      setScenePlan(payload.scenePlan);
      setAnalysisSource(payload.source ?? "mock");
      setPptxFilename(payload.siteTitle ?? trimmed);
      if (typeof payload.designMd === "string") {
        setDesignMd(payload.designMd);
      }
      setRenderStatus(
        `Analyzed ${payload.siteTitle ?? trimmed} · ${
          payload.source === "live" ? "Claude" : "deterministic fallback"
        }`,
      );
      resetAnimation();
      ...
      setRenderStatus(
        error instanceof Error ? error.message : "Site analysis failed.",
      );
    }
  }, [siteUrl, resetAnimation]);
    ...
    try {
      const response = await fetch(animateUrl, {
        ...
        body: JSON.stringify({
          ...
          styleIntent,
          motionPreset,
          // Free-form brand baseline (palette, type, voice). Server caps the
          // length; we forward the current contents and let it decide.
          designMd: designMd.trim().length > 0 ? designMd : undefined,
        }),
      });
                ...
                <aside className="cw-vs__rail">
                  <PptxSetupPanel
                    ...
                    onAnalysisComplete={(result) => {
                      ...
                      // .pptx flow doesn't generate a design.md today, but
                      // the URL flow can — propagate when present.
                      if (typeof result.designMd === "string") {
                        setDesignMd(result.designMd);
                      }
                      setRenderStatus(
